...
describe('normalizeGraphQueryArgs', () => {
  ...
  it('maps code/text to pattern for action=code', () => {
    expect(normalizeGraphQueryArgs({ action: 'code', code: 'TODO' }).pattern).toBe('TODO');
    expect(
  ...
  it('maps qualified_name to qn_pattern for action=search', () => {
    ...
    expect(result.qn_pattern).toBe('^\\.src\\.stores\\.products\\.getProductById$');
  });
...
describe('isLabelsTypesSchemaIntent', () => {
  ...
  it('detects type(r) edge discovery', () => {
    expect(isLabelsTypesSchemaIntent('MATCH ()-[r]->() RETURN DISTINCT type(r)')).toBe(true);
  });

  it('does not treat aggregation or multi-column RETURN as schema', () => {
    expect(isLabelsTypesSchemaIntent('MATCH ()-[r:CONTAINS]->() RETURN type(r), count(*)')).toBe(
      false
    );
...
describe('isAggregateSchemaIntent', () => {
  it('detects type(r) + count() aggregation (Bug #1)', () => {
    expect(isAggregateSchemaIntent('MATCH (n)-[r]->(m) RETURN type(r), count(*)')).toBe(true);
  });

  it('detects labels(n) + count() aggregation (Bug #2)', () => {
    expect(isAggregateSchemaIntent('MATCH (n) RETURN labels(n), count(n)')).toBe(true);
  });

  it('does not match non-aggregate type(r) queries', () => {
    expect(isAggregateSchemaIntent('MATCH (a)-[r]->(b) RETURN type(r) AS rel, a.name')).toBe(false);
  });
  ...
  it('does not match queries without type()/labels()', () => {
    expect(isAggregateSchemaIntent('MATCH (n) RETURN n.name, count(n)')).toBe(false);
  });
});
...
describe('normalizeCbmCypher', () => {
  it('rewrites relationshipType to type', () => {
    const { query } = normalizeCbmCypher('MATCH (a)-[r]->(b) RETURN relationshipType(r), a.name');
  ...
  it('rewrites WHERE labels(n) = label to n.label', () => {
    const { query } = normalizeCbmCypher('MATCH (n) WHERE labels(n) = "Function" RETURN n.name');
  ...
  it('rewrites labels(n)[0] and adds RETURN aliases for type/count', () => {
    const { query } = normalizeCbmCypher('MATCH ()-[r:IMPORTS]-() RETURN type(r), count(r) AS cnt');
  ...
  it('rewrites count(*) to count(relVar)', () => {
    const { query } = normalizeCbmCypher('MATCH ()-[r:CONTAINS]->() RETURN type(r), count(*)');
