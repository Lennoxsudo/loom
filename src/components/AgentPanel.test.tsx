...
async function selectProjectThread(user: ReturnType<typeof userEvent.setup>, title: string) {
...
beforeEach(() => {
  ...
  executeToolCallMock.mockImplementation(
    async (toolCall: { function?: { name?: string; arguments?: string } }) => {
...
test('AgentPanel 欢迎态 composer 可输入且不会自动发送', async () => {
  ...
  expect(invokeMock).not.toHaveBeenCalledWith('send_ai_chat_stream', expect.anything());
});
...
test('AgentPanel 发送首条消息时并行生成会话标题，不阻塞流式回复', async () => {
  ...
  expect(
    within(screen.getByTestId('agent-project-tree')).getByText('fix title issue')
  ).toBeInTheDocument();
...
test('AgentPanel auto-discards pending change when rollback target is missing', async () => {
  ...
  await waitFor(() => {
    expect(
      within(screen.getByTestId('change-review-panel')).getByText('demo.ts')
    ).toBeInTheDocument();
  });
...
test('AgentPanel 连续两次修改同一文件时保留首次 diff 基线', async () => {
  ...
  executeToolCallMock.mockImplementation(async (toolCall: { function: { name: string } }) => {
    if (toolCall.function.name === 'write_file') {
      writeCount += 1;
