...
describe('往返一致性 (serialize → parse → serialize)', () => {
  ...
  it('完全自定义状态往返后一致', () => {
    const original: Omit<SettingsState, 'loading'> = {
      ...
      foldersFirst: false,
      streamSpeed: 'fast',
      streamSendMode: 'interrupt_and_send',
      chatSendDuringStreamingMode: 'interrupt',
      ...
      enableAgentMemoryUserProfile: false,
      enableAgentMemoryNotes: false,
      enableAgentSessionSearch: false,
      enableAgentMemoryReview: true,
      agentMemoryWriteApproval: true,
