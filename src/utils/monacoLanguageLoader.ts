...
// 语言定义映射
const languageDefinitions: Record<string, {
  loader: () => Promise<any>;
...
export async function loadMonacoLanguage(language: string): Promise<void> {
  ...
  if (!definition) {
    console.warn(`[MonacoLoader] Language "${language}" (normalized: "${normalizedLang}") not supported for lazy loading`);
    return;
  }
...
export async function loadMultipleLanguages(languages: string[]): Promise<void> {
  const uniqueLangs = [...new Set(languages.map(lang => 
